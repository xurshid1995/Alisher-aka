...
# 3. Deploy via SSH
Write-Host ""
Write-Host "[3/3] Serverga deploy (207.154.225.70)..." -ForegroundColor Cyan
$sshCmd = "cd /var/www/alisher && git pull origin main && source venv/bin/activate && pip install -r requirements.txt -q && sudo systemctl restart alisher && sleep 2 && sudo systemctl status alisher --no-pager -n 5"
ssh root@207.154.225.70 $sshCmd
if ($LASTEXITCODE -ne 0) {
